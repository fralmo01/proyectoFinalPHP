    ...
    <div class="container mt-5 pt-5" style="position: relative; z-index: 1;">
        <div class="d-flex align-items-center mb-4">
            ...
            <div>
                ...
                <small class="text-muted">Gestiona las etapas, evaluaciones y decisiones finales de tus
                    candidatos.</small>
            </div>
        </div>
        ...
        <?php foreach ($bloques as $clave => $info): ?>
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                    <div>
                        <h4 class="mb-0"><i
                                class="fas <?= $info['icono'] ?> text-<?= $info['color'] ?> me-2"></i><?= htmlspecialchars($info['titulo']) ?>
                        </h4>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?php if (!empty($postulacion['etapa'])): ?>
                                                    <span
                                                        class="badge bg-light text-dark border"><?= htmlspecialchars($postulacion['etapa']) ?></span>
                                                <?php else: ?>
                                            ...
                                            + <td>
                                                + <?php if (!empty($postulacion['estadoResultado'])): ?>
                                                    + <span
                                                        class="badge bg-dark"><?= htmlspecialchars($postulacion['estadoResultado']) ?></span>
                                                    + <?php else: ?>
                                            ...
                                            <td>
                                                <div class="d-flex gap-2">
                                                    <a class="btn btn-outline-success btn-sm"
                                                        href="index.php?controller=Evaluacion&action=detalle&idPostulacion=<?= (int) $postulacion['idPostulacion'] ?>#form-evaluacion">
                                                        <i class="fas fa-clipboard"></i> Evaluar
                                                    </a>
                                                    <form method="post" action="index.php?controller=Evaluacion&action=listar"
                                                        class="d-inline">
                                                        <input type="hidden" name="accion" value="cambiar_etapa">
                                                        <input type="hidden" name="idPostulacion"
                                                            value="<?= (int) $postulacion['idPostulacion'] ?>">
                                                        <input type="hidden" name="idEtapaActual"
                                                            value="<?= (int) ($postulacion['idEtapa'] ?? 0) ?>">
                                                        <button type="submit" class="btn btn-outline-primary btn-sm"
                                                            <?= empty($postulacion['idEtapa']) ? 'disabled' : '' ?>>
                                                            <i class="fas fa-arrow-up"></i> Cambiar etapa
                                                        </button>
                                                    </form>
                                                    <a class="btn btn-outline-dark btn-sm"
                                                        href="index.php?controller=Evaluacion&action=detalle&idPostulacion=<?= (int) $postulacion['idPostulacion'] ?>#form-resultado">
