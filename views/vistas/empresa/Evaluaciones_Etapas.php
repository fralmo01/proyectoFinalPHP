                ...
                <div class="col-lg-5">
                    <div class="card shadow-sm border-0 h-100">
                        ...
                        <div class="card-body">
                            ...
                            <p class="mb-2">
                                <strong>Convocatoria:</strong><br><?= htmlspecialchars($detalle['convocatoria']) ?></p>
                            ...
                            <?php if (!empty($detalle['comentario'])): ?>
                                <p class="mb-2"><strong>Comentario del
                                        postulante:</strong><br><?= nl2br(htmlspecialchars($detalle['comentario'])) ?></p>
                            <?php endif; ?>
                            <p class="mb-2">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="idEtapaNueva" class="form-select" required>
                                            ...
                                            <?php foreach ($etapasPosteriores as $etapa): ?>
                                                <option value="<?= (int) $etapa['idEtapa'] ?>">
                                                    <?= htmlspecialchars($etapa['nombre']) ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    <button type="submit" class="btn btn-primary"><i class="fas fa-arrow-up me-2"></i>Actualizar
                                        etapa</button>
                                </form>
                            <?php else: ?>
                            ...
                            <form method="post"
                                ...
                                <div class="row g-3">
                                    <div class="col-md-8">
                                        ...
                                        <select name="idEstadoResultado" class="form-select" required>
                                            ...
                                            <?php foreach ($estadosResultado as $estado): ?>
                                                <option value="<?= (int) $estado['idEstadoResultado'] ?>"
                                                    <?= (!empty($resultado['idEstadoResultado']) && (int) $resultado['idEstadoResultado'] === (int) $estado['idEstadoResultado']) ? 'selected' : '' ?>>
                                    ...
                                    <div class="col-md-4 d-grid">
                                        <button type="submit" class="btn btn-dark"><i class="fas fa-check me-2"></i>Guardar
                                            decisión</button>
                                    </div>
                                </div>
                            </form>
                                    ...
                                    <?php foreach ($historialEtapas as $registro): ?>
                                        <li class="list-group-item px-0">
                                            ...
                                            <div class="text-muted small">
                                                <i
                                                    class="fas fa-clock me-1"></i><?= htmlspecialchars(date('d/m/Y H:i', strtotime($registro['fechaAccion']))) ?>
                                                <?php if (!empty($registro['usuario'])): ?>
                                                    &nbsp;|&nbsp;<i
                                                        class="fas fa-user me-1"></i><?= htmlspecialchars($registro['usuario']) ?>
                                                <?php endif; ?>
                                            </div>
                                        </li>
                                    <?php endforeach; ?>
                                    ...
                                    <table class="table table-sm table-hover align-middle">
                                        ...
                                        <tbody>
                                            <?php foreach ($evaluaciones as $evaluacion): ?>
                                                <tr>
                                                    <td><span
                                                            class="badge bg-success fs-6"><?= number_format((float) $evaluacion['puntaje'], 2) ?></span>
                                                    </td>
                                                    <td><?= !empty($evaluacion['observaciones']) ? nl2br(htmlspecialchars($evaluacion['observaciones'])) : '<span class="text-muted">Sin observaciones</span>' ?>
                                                    </td>
                                                    <td><?= htmlspecialchars(date('d/m/Y H:i', strtotime($evaluacion['fechaEvaluacion']))) ?>
                                                    </td>
